...
import { RIGHTS, CONTACTS, SOCIALS } from "../Data/footer.data";
        ...
        >
          {RIGHTS.map((item) => (
            <a
              key={item.label}
              href={item.href}
              className="text-center text-xl max-sm:text-sm hover:text-white transition-colors duration-200"
            >
              {item.label}
            </a>
          ))}
            ...
            <div key={label} className="flex items-center gap-2">
              ...
              {label === "Address" ? (
                <a
                  ...
                  className="text-xl max-sm:text-sm hover:text-white transition-colors"
