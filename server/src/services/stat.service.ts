...
/** 標準常態分佈的分位數（inverse CDF）。p ∈ (0,1) */
function quantileNormal(p: number) {
  return jStat.normal.inv(p, 0, 1);
}

/** 將數值夾在 [0,1] 範圍內（避免數值近似誤差讓 CI 越界）。 */
 ...
 * - 會輸出 t/df/p、兩組的大小與平均、Levene 等變異檢定、Cohen's d 與差異的 CI。
 * @param rows    資料列（每列為物件）
 * @param groupKey 群組欄位名（例如 treatment）
 * @param valueKey 數值欄位名（例如 score）
...
export function independentT(
  ...
  const [A, B] = keys as [string, string]; // tuple 斷言：避免 string|undefined
  if (!g[A] || !g[B]) throw { status: 400, code: "GROUP_UNDEFINED", message: "分組不存在" };

  // 2) 取出兩組數值並轉 number（含缺值/NaN 防呆）
...
/**
 * 成對樣本 t 檢定（以差值向量做單樣本 t）
 * - 會輸出 t/df/p、差值平均、Cohen's dz、差值的 CI。
 * @param preKey  前測欄位
 * @param postKey 後測欄位
 */
...
/**
 * 單因子 ANOVA
 * - 手刻 SS_between / SS_within，F 與 p。
 * - 回傳每組平均、樣本數、效果量 η² 與其保守 CI 近似（可後續換更嚴謹方法）。
 */
export function anovaOneWay(
  ...
  // 粗略近似：方便先有 CI；可視需要換成更嚴謹方法（如 Smithson 2001）
  const varEta2Approx = (2 * (eta2 ** 2) * (dfw ** 2 + (k - 1) ** 2)) / ((N ** 2) * (k - 1) ** 2);
  const z = quantileNormal(0.5 + (ciLevel / 2));
  const ciEta2: CI = {
    level: ciLevel,
...
export function correlation(
  ...
  // 3) Fisher z 轉換的 CI（r → z → CI → r）
  const zVal  = 0.5 * Math.log((1 + r) / (1 - r));
  const se    = 1 / Math.sqrt(n - 3);
  const zcrit = quantileNormal(0.5 + ciLevel / 2);
